...
func runPlugin(args []string) {
	...
	case "add":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "plugin add requires an https git URL")
			os.Exit(1)
		}
		load := ""
		if len(args) > 2 {
		...
		name, err := plugins.Add(cfg, args[1], load)
		if err != nil {
...
func suggestCommand(input string) string {
	commands := []string{"preview", "apply", "doctor", "reset", "theme", "header", "plugin", "tui", "version"}
