...
func TestInjectBlock_EmptyZshrc(t *testing.T) {
	withTempHome(t)
...
func TestInjectBlock_ReplacesExistingBlockWithoutDuplicate(t *testing.T) {
	withTempHome(t)
...
func TestRemoveBlock_RemovesOnlyOzshBlock(t *testing.T) {
	withTempHome(t)
...
func TestRemoveBlock_PreservesMissingFinalNewline(t *testing.T) {
	original := "export EDITOR=vim"
	content := injectBlockContent(original)

	got := removeBlock(content)
	if got != original {
...
func TestBackup_CreatesTimestampedBackup(t *testing.T) {
	home := withTempHome(t)
	...
	if matched := regexp.MustCompile(`zshrc-\d{4}-\d{2}-\d{2}-\d{4}\.bak$`).MatchString(filepath.Base(backupPath)); !matched {
