...
`)
	...
	b.WriteString("}\n")
	b.WriteString("\n")
	b.WriteString("precmd_functions+=(ozsh_prompt)\n")
...
func pluginSourcePath(plugin config.PluginItem) (string, bool) {
	...
	if plugin.Load != "" {
		if filepath.IsAbs(plugin.Load) {
			return "", false
		}
		clean := filepath.Clean(plugin.Load)
		...
		source = filepath.Join(plugin.Source, clean)
	}
	return source, true
}
...
func segmentActive(cfg *config.Config, name string, segment config.SegmentConfig) bool {
	...
	return !cfg.Prompt.DisableHeavySegments || !isHeavySegment(name)
}
...
func zshPromptSeparator(separator string) string {
	return strings.ReplaceAll(separator, ":", "\\:")
}
...
func usesHexColor(cfg *config.Config) bool {
	...
	themeColors := []string{cfg.Theme.Accent, cfg.Theme.Success, cfg.Theme.Warning, cfg.Theme.Error}
	for _, color := range themeColors {
		if strings.HasPrefix(color, "#") {
			return true
		}
	}
