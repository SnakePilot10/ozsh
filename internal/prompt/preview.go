...
func SimulatedWithContext(cfg *config.Config, previewCtx PreviewContext) string {
	...
	for _, name := range cfg.Prompt.Order {
		rendered := renderPreviewSegment(cfg, ctx, name)
		if rendered != "" {
	...
	line := strings.Join(parts, cfg.Prompt.Separator)
	if cfg.Prompt.RightPrompt || len(rightParts) > 0 {
		line = fmt.Sprintf("%s    %s", line, strings.Join(rightParts, cfg.Prompt.Separator))
	}
