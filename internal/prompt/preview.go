...
func SimulatedWithContext(cfg *config.Config, previewCtx PreviewContext) string {
    // clone config to avoid mutating the original when filling defaults
    clone := *cfg
    // deep copy segments map
    clone.Prompt.Segments = make(map[string]config.SegmentConfig)
    for k, v := range cfg.Prompt.Segments {
        clone.Prompt.Segments[k] = v
    }
