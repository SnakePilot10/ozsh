...
func TestGenerate_DoesNotReadTemplateFromWorkingDirectory(t *testing.T) {
	...
	if !strings.Contains(output, "ozsh_prompt()") {
		t.Fatalf("Generate() did not use the complete built-in generator:\n%s", output)
	}
}

func TestGenerateRendersIconAndBackground(t *testing.T) {
	cfg := config.Default()
	segment := cfg.Prompt.Segments["user"]
	segment.Icon = "@"
	segment.BG = "blue"
	cfg.Prompt.Segments["user"] = segment
	...
	if !strings.Contains(preview, "@ snake") || !strings.Contains(preview, "\x1b[44m") {
		t.Fatalf("Simulated() missing icon/background styling: %q", preview)
	}
}

func TestGenerate_HostileIconDoesNotExecute(t *testing.T) {
	if _, err := exec.LookPath("zsh"); err != nil {
		t.Skip("zsh not available")
	}
	tmp := t.TempDir()
	dollarSentinel := filepath.Join(tmp, "icon-dollar-pwned")
	backtickSentinel := filepath.Join(tmp, "icon-backtick-pwned")

	cfg := config.Default()
	segment := cfg.Prompt.Segments["user"]
	segment.Icon = "$(touch " + dollarSentinel + ") `touch " + backtickSentinel + "` %n !"
	cfg.Prompt.Segments["user"] = segment
	output, err := Generate(cfg)
	if err != nil {
		t.Fatalf("Generate() error = %v", err)
	}
	script := filepath.Join(tmp, "icon.zsh")
	harness := output + `
setopt prompt_subst prompt_percent prompt_bang
ozsh_prompt
print -r -- "${(%)PROMPT}" >/dev/null
`
	if err := os.WriteFile(script, []byte(harness), 0o600); err != nil {
		t.Fatalf("WriteFile(icon script) error = %v", err)
	}
	cmd := exec.Command("zsh", script)
	cmd.Env = append(os.Environ(), "HOME="+tmp)
	if out, err := cmd.CombinedOutput(); err != nil {
		t.Fatalf("zsh icon render failed: %v\n%s\n%s", err, out, output)
	}
	for _, sentinel := range []string{dollarSentinel, backtickSentinel} {
		if _, err := os.Stat(sentinel); !os.IsNotExist(err) {
			t.Fatalf("hostile icon executed command and created %s", sentinel)
		}
	}
}
